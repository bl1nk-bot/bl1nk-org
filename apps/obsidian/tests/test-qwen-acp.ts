...
async function testAgent(agentPath: string, agentName: string): Promise<TestResult> {
  ...
  try {
    ...
    const outputStream = Writable.toWeb(proc.stdin) as WritableStream<Uint8Array>
    const inputStream = Readable.toWeb(proc.stdout) as ReadableStream<Uint8Array>

    const stream = ndJsonStream(outputStream, inputStream)

    const toClient = (_agent: Agent): Client => ({
      async requestPermission(params: unknown) {
        console.log(`[${agentName}] Permission requested:`, JSON.stringify(params, null, 2))
    ...
    console.log(`[${agentName}] Initialized:`, JSON.stringify(initResult, null, 2))
