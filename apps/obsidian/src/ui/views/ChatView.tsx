import { AlertCircle, AtSign, Bot, Loader2, Send, Slash, User, Wrench } from "lucide-react"
...
const EnhancedChatComponent: React.FC<{ plugin: SmartAssistantPlugin }> = ({ plugin }) => {
  ...
  useEffect(() => {
    ...
    const handleConnected = (data: { agentName: string; agentVersion: string }) => {
    ...
    if (info) {
      ...
      setError("ACP Agent ยังไม่ได้เชื่อมต่อ กรุณาไปที่ Settings → เปิดใช้งาน agent ก่อน")
    }
  ...
  const handleKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === "Enter" && !e.shiftKey) {
      ...
      if (showFileSuggestions) {
        ...
        setSelectedCommandIndex((prev) => Math.min(prev + 1, commandSuggestions.length - 1))
      }
  ...
  return (
    <div className="osa-chat-view flex flex-col h-full bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100">
      ...
      <div className="osa-chat-messages flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {messages.length === 0 && !error && (
          <div className="flex flex-col items-center justify-center h-full text-center text-zinc-500 dark:text-zinc-400">
            ...
            <p className="text-xs mt-3 opacity-60">ใช้ @ เพื่อ mention ไฟล์ หรือ / เพื่อใช้ commands</p>
          </div>
        )}
      ...
      <div className="osa-chat-input p-4 border-t border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
        ...
        {isLoading && <div className="text-xs text-zinc-500 mt-1 pl-1">กำลังประมวลผล...</div>}
      </div>
    </div>
  )
}
...
export class EnhancedChatView extends ItemView {
  ...
  async onOpen() {
    ...
    this.root.render(React.createElement(EnhancedChatComponent, { plugin: this.plugin }))
  }
