          ...
          this.app.workspace.on("quit", () => {
            ...
            for (const [viewId, adapter] of this._adapters) {
              adapter.disconnect().catch((error) => {
                console.warn(
                  `[AgentClient] ข้อผิดพลาด cleanup สำหรับ view ${viewId}:`,
                  error,
                );
              });
            }
      ...
      try {
        ...
        for (const [viewId, adapter] of this._adapters) {
          try {
            ...
            console.warn(
              `[AgentClient] ข้อผิดพลาดระหว่างปิด adapter สำหรับ view ${viewId}:`,
              error,
            );
          }
        }
    ...
    async removeAdapter(viewId: string): Promise<void> {
      ...
      if (adapter) {
        try {
          ...
          console.warn(
            `[AgentClient] ล้มเหลวในการปิด adapter สำหรับ view ${viewId}:`,
            error,
          );
        }
        ...
        if (isCurrentPrerelease) {
          ...
          if (hasNewerStable || hasNewerPrerelease) {
            ...
            const newestVersion = hasNewerStable
              ...
              : latestPrerelease;
            new Notice(
              `[AgentClient] มีการอัปเดต: v${newestVersion}`,
            );
            return true;
          }
        } else {
