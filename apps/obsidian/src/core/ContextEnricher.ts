...
import type { KnowledgeManager } from "./KnowledgeManager"
...
export class ContextEnricher {
  ...
  async enrich(text: string, activeFile: TFile | null): Promise<string> {
    ...
    if (this.settings.acp.contextMode === "rag_enriched" && this.settings.rag.enabled) {
      const ragResults = await this.knowledgeManager.query({ query: text })
      if (ragResults.length > 0) {
        const ragText = ragResults.map((r) => `- ${r.title}: ${r.excerpt}`).join("\n")
